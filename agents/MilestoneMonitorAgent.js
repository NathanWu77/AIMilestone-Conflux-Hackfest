...
class MilestoneMonitorAgent {
  ...
  async start() {
    ...
    try {
      ...
      console.log("   - deposit     : Deposit USDT0 to contract");
      console.log("   - create milestone : Create new milestone");
      console.log("   - proof       : Submit proof (auto-analyzes & recommends)");
      console.log("   - approve     : Approve milestone");
  ...
  create milestone <amount> <deadline> <desc>  - Create milestone (e.g., "create milestone 50 2026-06-01 Implement login")
...
💡 Example Usage:
  ...
  - "create milestone 50 2026-06-01 Implement user authentication" - Create milestone
  - "proof https://github.com/user/repo/pull/123" - Submit proof (auto-analyzes)
...
💡 Next Step: Create milestones using "create milestone <amount> <deadline> <description>"`;
      ...
      return `❌ Error depositing USDT0: ${error.message}

💡 Tips:
  - Make sure you have USDT0 tokens in your wallet
  - Try minting tokens first: Run the deploy script which includes minting
  - Check your balance using the "status" command`;
    }
  }
...
💡 Suggestions:
  ...
  - View all milestones: "milestones"`;
      ...
      if (analysis.recommend) {
        // Auto-submit the recommendation to blockchain
...
💡 Tips:
  ...
  analyzeProofContent(proofText, pendingMilestones) {
    ...
    const completionIndicators = [
      ...
      'tested', 'merged', 'released', 'delivered'
    ];
    ...
    if (hasGitHubLink && hasCompletionIndicator) {
      ...
      if (hasQualityIndicator) {
        ...
        recommend = true;
        reasoning += `The proof shows concrete evidence of milestone completion with proper quality checks. `;
      } else {
        reasoning += `⚠️  Limited quality indicators found. `;
        suggestion = "Consider adding information about testing, code review, or documentation.";
      }
    } else if (hasCompletionIndicator) {
      reasoning += `✅ Completion indicators detected. `;
      reasoning += `⚠️  No GitHub link provided for verification. `;
      suggestion = "Please provide a GitHub PR or commit link for verification.";
    } else if (hasGitHubLink) {
      reasoning += `✅ GitHub link provided. `;
      reasoning += `⚠️  Clear completion statement not found. `;
      suggestion = "Please confirm that the milestone requirements have been met.";
    } else {
      reasoning += `⚠️  Limited proof information provided. `;
      suggestion = "Please provide more details including GitHub links and completion confirmation.";
    }
...
⚡ Gas sponsored - zero cost transaction!
🔗 View on ConfluxScan: https://evmtestnet.confluxscan.io/tx/${receipt.transactionHash}
